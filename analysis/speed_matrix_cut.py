...
def average_speed_file(filename):
    ...
    u_mean = u_sum / u_count if u_count != 0 else 0
    v_mean = v_sum / v_count if v_count != 0 else 0
...
if __name__ == "__main__":
    ...
    np.savetxt(destination+"horizontal_speed_matrix_new_cut.csv", np.array(horizontal_speed), delimiter=',')
    np.savetxt(destination+"vertical_speed_matrix_new_cut.csv", np.array(vertical_speed), delimiter=',')
    np.savetxt(destination+"offset_err_cut.csv", np.array(offset_err), delimiter=',')
    np.savetxt(destination+"temperature_err_cut.csv", np.array(temperature_err), delimiter=",")